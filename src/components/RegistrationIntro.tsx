...
export const RegistrationIntro: React.FC<RegistrationIntroProps> = ({
  ...
  const handleSaveParentSettings = (e: React.FormEvent) => {
    ...
    onUpdateFamily(updatedFam);
    setSettingsSavedMsg('Đã cập nhật cài đặt Phụ huynh & Mật khẩu thành công!');
  ...
  const handleStudentLogin = (e: React.FormEvent) => {
    ...
    const currentCode = (family.familyCode || '').toUpperCase().replace(/[^A-Z0-9]/g, '');

    if (currentCode && inputCode === currentCode && family.children && family.children.length > 0) {
      const selectedChild = family.children.find(c => c.id === studentChildId) || family.children[0];
    ...
    setStudentError('Mã Gia Đình chưa chính xác hoặc chưa có hồ sơ học sinh!');
  };

  // Handle Parent Action (Login or Create)
  const handleParentAction = (e: React.FormEvent) => {
    e.preventDefault();
    setParentError('');
            ...
            <form onSubmit={handleStudentLogin} className="space-y-4 pt-1 animate-in fade-in duration-200">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="Nhập mã gia đình (VD: GD-DEMO)"
              ...
              {isFamilyCodeValid ? (
                <div className="space-y-3 animate-in fade-in zoom-in-95 duration-150">
                  ...
                  <div className="p-2.5 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center justify-between text-xs">
                    ...
                    <span className="text-emerald-700 font-mono text-[11px] font-bold bg-white px-2 py-0.5 rounded border border-emerald-200">
                      {family.familyCode}
                    </span>
                  </div>
